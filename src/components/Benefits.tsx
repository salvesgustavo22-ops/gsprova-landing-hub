        ...
        <div className="mx-auto grid max-w-6xl gap-8 md:grid-cols-3">
          {benefits.map((benefit, index) => (
            <div key={index} className="group text-center">
              <div className="mb-6 inline-flex size-16 items-center justify-center rounded-xl bg-gradient-to-br from-primary to-secondary shadow-lg transition-transform duration-300 group-hover:scale-110">
                <benefit.icon className="size-8 text-white" />
              </div>

              <h3 className="mb-3 text-xl font-bold text-white">{benefit.title}</h3>

              <p className="font-light leading-relaxed text-white/85">{benefit.description}</p>
            </div>
          ))}
        </div>
      </div>
